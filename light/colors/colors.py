...
def color_palette(name=None, n_colors=6, desat=None):
    ...
    Available seaborn palette names: deep, muted, bright, pastel, dark,
    colorblind
    Other options: hls, husl, any matplotlib palette
    Matplotlib palettes can be specified as reversed palettes by appending
    "_r" to the name or as dark palettes by appending "_d" to the name.
    This function can also be used in a with-statement to temporarily
    set the color cycle for a plot or set of plots.

    @param name: None, string, or sequence
        Name of palette or None to return current palette. If a
        sequence, input colors are used but possibly cycled and
        desaturated.
    @param n_colors: A C{int} number of colors in the palette. If larger than
        the number of colors in the palette, they will cycle.
    @param desat: A C{float} value to desaturate each color by.
    @return: A C{list} of RGB tuples.
    """
