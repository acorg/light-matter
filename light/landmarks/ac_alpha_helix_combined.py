...
def _loadDatabase(filename):
    """
    Read alpha helix combined strings and add them to an Aho Corasick matcher.

    @param filename: A C{str} filename of a file with alpha helix combined.
...
# AC_AlphaHelix_combined.find method.
_AC = None

# Store the ac alpha helix combined filename for checking.
# For testing, we want to prevent the if statement in find() from running.
# Therefore, we set _STORED_AC_FILENAME to 'xxx', so that
# self._dbParams.acAlphaHelixCombinedFilename can also be set to 'xxx'.
_STORED_AC_FILENAME = 'xxx'


class AC_AlphaHelix_combined(Finder):
    """
    A class for finding alpha helix combined using Aho Corasick to store
    helices.
    """
    NAME = 'AC AlphaHelix_combined'
    SYMBOL = 'ACAHC'
    ...
    def find(self, read):
        ...
        if _AC is None or (
                _STORED_AC_FILENAME !=
                self._dbParams.acAlphaHelixCombinedFilename):
            _AC = _loadDatabase(self._dbParams.acAlphaHelixCombinedFilename)
            _STORED_AC_FILENAME = self._dbParams.acAlphaHelixCombinedFilename
