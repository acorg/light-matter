...
class Hydropathy(Finder):
    """
    A class for computing trig points based on similarity in hydropathy.
    """
    NAME = 'Hydropathy'
    SYMBOL = 'H'

    def convertAAToHydropathyClusters(self, sequence):
        """
        Takes an amino acid sequence, converts it to a sequence of numbers
        corresponding to the cluster an amino acid hydropathy is in.
        ...
        """
        return [PROPERTY_CLUSTERS[aa]['hydropathy'] if aa in PROPERTY_CLUSTERS
                else 0 for aa in sequence]

    def find(self, read):
        """
        A function that checks if and where a change of hydropathy in a
        sequence occurs.

        @param read: An instance of C{dark.reads.AARead}.
        """
        propertySequence = self.convertAAToHydropathyClusters(read.sequence)
