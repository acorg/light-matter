...
class Hydroxythiolation(Finder):
    """
    A class for computing trig points based on similarity in hydroxythiolation.
    """
    NAME = 'Hydroxythiolation'
    SYMBOL = 'HT'

    def convertAAToHydroxythiolationClusters(self, sequence):
        """
        Takes an amino acid sequence, converts it to a sequence of numbers
        corresponding to the cluster an amino acid hydroxythiolation is in.
        ...
        """
        return [PROPERTY_CLUSTERS[aa]['hydroxythiolation'] if aa in
                PROPERTY_CLUSTERS else 0 for aa in sequence]

    def find(self, read):
        """
        A function that checks if and where a change of hydroxythiolation in a
        sequence occurs.

        @param read: An instance of C{dark.reads.AARead}.
        """
        propertySequence = self.convertAAToHydroxythiolationClusters(
            read.sequence)
