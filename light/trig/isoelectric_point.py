...
class IsoelectricPoint(Finder):
    """
    A class for computing trig points based on similarity in the isoelectric
    Point.
    """
    NAME = 'IsoelectricPoint'
    SYMBOL = 'IEP'

    def convertAAToIEPClusters(self, sequence):
        """
        Takes an amino acid sequence, converts it to a sequence of numbers
        corresponding to the cluster an amino acid iep is in.
        ...
        """
        return [PROPERTY_CLUSTERS[aa]['iep'] if aa in PROPERTY_CLUSTERS
                else 0 for aa in sequence]

    def find(self, read):
        """
        A function that checks if and where a change of iep in a
        sequence occurs.

        @param read: An instance of C{dark.reads.AARead}.
        """
        propertySequence = self.convertAAToIEPClusters(read.sequence)
