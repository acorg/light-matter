"""
This tests are adapted from
http://code.activestate.com/recipes/577197-sortedcollection/

The code is pretty much as found above. It has not been adapted much to
match our style or to use the TestCase assert methods.
"""
...
class TestSortedCollection(TestCase):
    """
    Tests for the light.sortedCollection.SortedCollection class
    """

    def testSortedCollection(self):
        ...
        for i in range(50):
            for n in range(6):
                ...
                s.sort()
                for probe in pool:
                    assert (repr(ve2no(sc.index, probe)) ==
                            repr(slow_index(s, probe)))
                    assert (repr(ve2no(sc.find, probe)) ==
                            repr(slow_find(s, probe)))
                    assert (repr(ve2no(sc.find_le, probe)) ==
                            repr(slow_find_le(s, probe)))
                    assert (repr(ve2no(sc.find_lt, probe)) ==
                            repr(slow_find_lt(s, probe)))
                    assert (repr(ve2no(sc.find_ge, probe)) ==
                            repr(slow_find_ge(s, probe)))
                    assert (repr(ve2no(sc.find_gt, probe)) ==
                            repr(slow_find_gt(s, probe)))
                ...
                assert (list(map(repr, reversed(sc))) ==
                        list(map(repr, reversed(s))))  # test __reversed__
                assert list(sc.copy()) == list(sc)  # test copy()
                sc.clear()  # test clear()
                assert len(sc) == 0

        sd = SortedCollection('The quick Brown Fox jumped'.split(),
                              key=str.lower)
        assert sd._keys == ['brown', 'fox', 'jumped', 'quick', 'the']
        assert sd._items == ['Brown', 'Fox', 'jumped', 'quick', 'The']
        assert sd._key == str.lower
        assert (repr(sd) ==
                "SortedCollection(['Brown', 'Fox', 'jumped', 'quick', 'The'], "
                "key=lower)")
        sd.key = str.upper
        ...
        assert sd._keys == ['BROWN', 'FOX', 'JUMPED', 'JUMPED', 'QUICK',
                            'QUICK', 'THE']
        assert sd._items == ['Brown', 'Fox', 'jUmPeD', 'jumped', 'quick',
                             'QuIcK', 'The']
    ...
    def testDoctests(self):
        """
        Run the SortedCollection doctests.
        """
        if doctest.testmod(light.sortedCollection).failed:
            self.fail('SortedCollection docstring tests failed.')
